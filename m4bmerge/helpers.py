...
import collections, logging, os, requests
	...
	for EXT in EXTENSIONS:
		if collections.Counter(
			...
			if sum(
				...
				for m4b_file in Path(path_to_check).glob(f'*.{extension_to_use}'):
					logging.debug(f"Adjusted input for {path_to_check} to use single m4b file")
